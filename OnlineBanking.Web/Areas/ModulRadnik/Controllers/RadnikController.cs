    ...
    public class RadnikController : Controller {
        ...
        public ActionResult EditRadnikProfil(LicniPodaciVM model, HttpPostedFileBase file) {
            ...
            if (file == null)
                ...
                KorisnikLoginVM korisnik = Autentifikacija.GetLogiraniKorisnik(HttpContext);
                korisnik.SlikaPath = ImageName;
            }
        ...
        public ActionResult EditKlijentProfil(int KlijentId) {
            ...
            model.IsActive = !klijent.IsDeleted;
            return PartialView(model);
        }
        ...
        public ActionResult SnimiKlijentProfil(EditLicniPodaciKlijentVM model, HttpPostedFileBase file) {
            ...
            string poruka = "";
            if (klijent.IsDeleted != !model.IsActive) {
                if (model.IsActive) {
                    poruka += "Poštovani <h3> " + klijent.Ime + " " + klijent.Prezime + ",</h3><br/>";
                    poruka += "Vaš korisnički račun je ponovo aktiviran.<br />";
                    poruka += "<br /><br />Za dodatne informacije obratite se Vašoj obližnjoj poslovnici.<br /> Vaša banka<br />";
                    Email.Posalji("FIT Banka", poruka, klijent.Email);
                }
                else {
                    poruka += "Poštovani <h3> " + klijent.Ime + " " + klijent.Prezime + ",</h3><br/>";
                    poruka += "Vaš korisnički račun je deaktiviran.<br />";
                    poruka += "<br /><br />Za dodatne informacije obratite se Vašoj obližnjoj poslovnici.<br /> Vaša banka<br />";
                    Email.Posalji("FIT Banka", poruka, klijent.Email);
                }
            }
            klijent.Id = model.KlijentId;
            klijent.IsDeleted = !model.IsActive;
            klijent.BrojUlice = model.BrojUlice;
            klijent.Ulica = model.Ulica;
            klijent.Ime = model.Ime;
            klijent.Prezime = model.Prezime;
        ...
        public ActionResult PrikaziKorisnickeRacune() {
            List<Klijent> klijenti = db.Klijenti.ToList();
